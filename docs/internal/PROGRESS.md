...
**Branch:** `main`  
**Versi terbaru:** v2.40.3 (14 Sep — guard 7 titik stamp versi di CI + fix anti-revert stamp cabang + guard peta cabang→DB; deploy live & E2E ACC berjenjang dgn foto lulus) · v2.40.2 (14 Sep — fix foto bukti form) · v2.40.1 (14 Sep — fix penanda terlambat & submit Driver) · v2.40.0 (13 Sep — batas waktu submit OT) — riwayat lengkap semua versi: **CHANGELOG.md** — program 6 tahap ISO 27001 SELESAI
...
| Aspek | Status |
|-------|--------|
| Versi | **v2.40.3 (14 Sep)** — guard stamp versi + anti-revert + guard peta cabang→DB (SBY = master) + deploy live; sebelumnya v2.40.2 (fix foto bukti form), v2.40.1 (fix penanda terlambat & submit Driver), v2.40.0 (batas waktu submit OT) — riwayat lengkap di CHANGELOG.md |
...
| Deploy v2.40.3 + E2E alur ACC lengkap (14 Sep) | ✅ **LIVE & LULUS** — rebuild `bbm_web`: health ok, sw.js `bpf-spa-20260914-v2.40.3`, **fix anti-revert terbukti di produksi**: 8 DB cabang otomatis ter-stamp v2.40.3 dari kode saat boot tanpa update manual `branches` (master + tabel `branches` di-stamp via `save_company_identity` + UPDATE). E2E alur ACC berjenjang dgn foto asli: submit Driver/OB/Security (2 foto per pengajuan) → 200 + jurnal ACC pending; list GA HR memuat baris form + path foto; preview foto 200 image (sesi); **PATCH selama pending → 409 `SUPERVISOR_APPROVAL_REQUIRED`**; ACC-1 GA HR → step 2 admin; ACC-2 Admin → status final `approved`; PATCH setelah final → 200 (gate terbuka); **PDF Formulir OT per pengajuan (driver & ob) embed 2 JPEG `/DCTDecode`**. Catatan: field foto di SEMUA submit (incl. Driver) = `foto_mulai`/`foto_selesai` (validator shared) — probe pertama memakai `foto_mulai_b64` utk driver (salah skrip, bukan bug app). Cleanup ber-jejak: 4 baris OT + 4 jurnal ACC + 4 registri PDF + 5 user + 6 file foto dihapus; `e2e_fin_tmp` (sisa 7 Sep, masih aktif) dinonaktifkan mengikuti preseden `e2e_admin_tmp`; 11 jurnal pending pengaju asli (Edwin/Febri, pagi 14 Sep) dibiarkan |
| Konvensi peta cabang→DB (14 Sep) | ✅ **GUARD + DOKUMENTASI — TANPA RENAME (keputusan user: tidak ambil risiko)** — peta resmi: **SBY = DB master `bpf_asset_system`** (pola "cabang default = DB utama", seed init.sql baris 406); cabang lain `bpf_branch_<kode>`: BDG/BJM/JKT/JKT2/LPG/MLG/MDN/SMG aktif, JMB/PKU/PLM/PTK inactive (JMB & PKU/PTK belum punya DB); resolve via `resolve_db_name()` → `branches.db_name` (cache 60 dtk), **0 hardcode** di kode. Guard baru `scripts/check_branch_db_map.py` (step CI "Check branch DB map" mode statik: seed init.sql + anti-hardcode literal `bpf_branch_*` di app.py/modules/scripts; `--with-db` di server: validasi 13 cabang live, split-brain & DB hilang tertangkap) + `modules/branch_db_map.py` + **17 test** (`tests/test_branch_db_map_guard.py`, termasuk kontrol negatif scanner). Grant `bpf_user` wildcard `bpf\_%`.* — DB baru berawalan bpf_ otomatis tercakup. DB uji-restore `bpf_restore_test` (38 tabel, 7,2MB — sisa drill restore 4 Sep) di-drop 14 Sep dgn dump pengaman `/tmp/bpf_restore_test_backup_20260914.sql` (4,7MB); DB tersisa = master + 9 `bpf_branch_*` (8 aktif + PLM inactive) |
